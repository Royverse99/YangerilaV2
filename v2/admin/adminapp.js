...
/* YCS expected fields (for old ordering detection) */
const YCS_FIELDS = [
  'Timestamp (IST)',
  'Full Name',
  'Phone',
...
/* Heuristics: if many expected YCS fields exist, use whitelist ordering */
function isLikelyYcsRow(row){
  const keys = new Set(Object.keys(row||{}).map(k=>String(k||'').toLowerCase().trim()));
  let hits = 0;
  for (const label of YCS_FIELDS) if (keys.has(label.toLowerCase().trim())) hits++;
  return hits >= 6;
}
...
function isCourseColumn(name){
  ...
  return n === 'course' || n === 'coursename' || n === 'courseselected' || n === 'courseselected' || n === 'coursename*' || n === 'courseselected' || n === 'course name' || n === 'courseselected';
}
...
/* YCS renderer: if enough expected fields map, render in that order (minus Course); else render sheet order minus Course */
function renderTableYCS(rows, tableId){
  ...
  if (mappedCount >= USE_WHITELIST_THRESHOLD) {
    const filtered = resolved.filter(r => r.key && !isCourseColumn(r.key));
    thead.innerHTML = `<tr>${filtered.map(c => `<th>${esc(c.label)}</th>`).join('')}</tr>`;
    tbody.innerHTML = rows.map(r => `<tr>${filtered.map(c => ycsCellHtml(c.key, r[c.key])).join('')}</tr>`).join('');
  } else {
